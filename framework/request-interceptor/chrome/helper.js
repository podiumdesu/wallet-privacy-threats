...
const waitForNavigation = async (page, maxWaitTimeInMillisecs) => {
  ...
  try {
    ...
    await page.waitForNavigation({
      timeout: maxWaitTimeInMillisecs,
      waitUntil: "load",
    });
...
async function importMetaMaskWallet(logger, page) {
  ...
  // Type passphrase
  for (let i = 0; i < 12; i++) {
    const fill_phrase = await page.waitForXPath(
      '//*[@id="import-srp__srp-word-' + i + '"]',
    );
  ...
  const all_done_button = await page.waitForXPath(
...
async function connectMetaMaskWallet(logger, page, browser, args) {
  ...
  try {
    const button = await page.waitForXPath(`//*[text()="Maybe later"]`, {
      timeout: 200,
    });
    await button.click();
  } catch {}
  try {
    const button = await page.waitForXPath(`//*[text()=" I Know "]`, {
      timeout: 200,
    });
    await button.click();
  } catch {}
  ...
  // Click menu button
  try {
    const button = await page.waitForXPath(`//*[text()="account_cicle"]`, {
      timeout: 200,
    });
    await button.click();
  } catch {}
  ...
  for (const connect_button_string of connect_buttons) {
    try {
      const connect_button = await page.waitForXPath(
        `//*[text()="${connect_button_string}"]`,
        { timeout: 200 },
      );
      ...
      try {
        ...
        const connect_button = await page.waitForXPath(
          `//*[normalize-space(text())="${connect_button_string}"]`,
          { timeout: 200 },
        );
  ...
  if (connect == "") {
    for (const connect_button_string of connect_buttons) {
      try {
        let connect_button = await page.$x(
          `//button[contains(text(), "${connect_button_string}")]`,
        );
  ...
  // Close let's get started dialogs
  try {
    const button = await page.waitForXPath(`//*[text()="Get started"]`, {
      timeout: 200,
    });
    await button.click();
  } catch {}
  ...
  while (true) {
    ...
    if (metamask == "") {
      ...
      for (const metamask_button_string of metamask_buttons) {
        ...
        try {
          const button = await page.$x(
            `//button[contains(text(), "${metamask_button_string}")]`,
          );
          ...
          try {
            const metamask_button = await page.waitForXPath(
              `//*[(text())="${metamask_button_string}"]`,
              { timeout: 200 },
            );
            const rect = await page.evaluate((el) => {
              const { x, y } = el.getBoundingClientRect();
      ...
      if (connect != "") {
        const connect_button = await page.waitForXPath(connect, {
          timeout: 200,
        });
        const rect = await page.evaluate((el) => {
          const { x, y } = el.getBoundingClientRect();
      ...
      try {
        if (typeof metamask === "string" || metamask instanceof String) {
          const metamask_button = await page.waitForXPath(metamask, {
            timeout: 200,
          });
          const rect = await page.evaluate((el) => {
            const { x, y } = el.getBoundingClientRect();
    ...
    try {
      let checkbox = await page.waitForXPath('//input[@type="checkbox"]', {
        timeout: 500,
      });
  ...
  if (reason == "timeout" && pages.length <= 2) {
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        if (
          !pages[i].url().startsWith("chrome-extension://") &&
          !pages[i].url().includes(domain)
        ) {
          pages[i].close();
        }
      }
  ...
  if (pages.length > 2) {
    ...
    for (const p of pages) {
      if (p.url().includes("home.html")) {
        popup_url =
          p.url().slice(0, p.url().lastIndexOf("home.html")) +
          "notification.html";
  ...
  try {
    const next_button = await popup.waitForXPath(
    ...
    const continue_button = await popup.waitForXPath(
  ...
  try {
    const button = await page.waitForXPath(`//*[text()="Continue"]`, {
      timeout: 200,
    });
    await button.click();
  } catch {}
  ...
  try {
    ...
    const sign_button = await popup.waitForXPath(
  ...
  try {
    const approve_button = await popup.waitForXPath(`//*[text()="Approve"]`, {
      timeout: 200,
    });
    await approve_button.click();
    const switch_button = await popup.waitForXPath(
      `//*[text()="Switch network"]`,
      { timeout: 200 },
    );
