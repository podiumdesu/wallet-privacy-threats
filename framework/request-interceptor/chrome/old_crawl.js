...
const crawl = async (args) => {
  ...
  try {
    ...
    browser.on(
      "targetcreated",
      onTargetCreated.bind(undefined, args, log, webSockets, cdpClients),
    );
    ...
    if (args.url == undefined) {
      ...
      // Import wallet
      console.log("\033[94mImporting wallet...\033[0m");
      try {
    ...
    logger.debug(
      "\033[91mCaught error when crawling: for " + log.msg + "\033[0m",
    );
  }
  ...
  try {
    ...
    logger.debug(
      "\033[91mError when shutting down: " + e.toString() + "\033[0m",
    );
  }
