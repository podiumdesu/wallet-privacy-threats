...
class Hasher():
    def __init__(self):
        ...
        # Use PyCryptodome’s MD4 implementation directly:
        from Crypto.Hash import MD4
        hashes['md4'] = lambda x: MD4.new(x.encode()).hexdigest()
        ...
        hashes['ripemd160'] = lambda x: RIPEMD160.new(x.encode()).hexdigest()
        # hashes['whirlpool']  = lambda x: WHIRLPOOL.new(x.encode()).hexdigest()
        hashes['sha_salted_1'] = lambda x: hashlib.sha256(x.encode() + 'QX4QkKEU'.encode()).hexdigest()
    ...
    def substring_search(self, input_string, max_layers=None, prev_encodings=tuple()):
        ...
        for n_precomp_layer in range(1, n_max_precomp_layer + 1):
            ...
            for transform_stack, string in _precompute_pool.items():
                if string in input_string or string.lower() in input_lower:
